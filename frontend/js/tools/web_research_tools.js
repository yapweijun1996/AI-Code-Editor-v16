...
async function _readUrl({
  ...
  for (let attempt = 1; attempt <= retries + 1; attempt++) {
    ...
    try {
      const response = await fetch("http://localhost:3333/api/read-url", {
        method: "POST",
      ...
      const message =
        payload?.message || `HTTP ${response.status} from /api/read-url`;
      ...
      const isTransient =
        /5\d\d/.test(errMsg) ||
        /ECONNRESET|ENOTFOUND|EAI_AGAIN|timeout|NetworkError|Failed to fetch/i.test(
          errMsg,
        );

      console.warn(
        `[read_url] ${attemptInfo} failed for ${url}: ${errMsg}${isAbort ? " (timeout)" : ""}`,
      );
      ...
      // Final failure
      throw new Error(
        `Failed to read URL after ${retries + 1} attempts: ${errMsg}`,
      );
    }
  }
}
...
async function _duckduckgoSearch({
  ...
  if (!query || typeof query !== "string") {
    throw new Error(
      "Failed to perform search: query must be a non-empty string",
    );
  }
  ...
  for (let attempt = 1; attempt <= retries + 1; attempt++) {
    ...
    try {
      const response = await fetch(
        "http://localhost:3333/api/duckduckgo-search",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      try {
        ...
        const text = await response.text().catch(() => "");
        payload = {
          message: text || "Non-JSON response from /api/duckduckgo-search",
        };
      }
      ...
      if (response.ok) {
        ...
        console.log(
          `[duckduckgo_search] Success for "${query}". Results: ${resultCount}`,
        );
        if (resultCount > 0) {
      ...
      const message =
        payload?.message ||
        `HTTP ${response.status} from /api/duckduckgo-search`;
      throw new Error(message);
    } catch (err) {
      clearTimeout(timeout);
      if (parentSignal)
      ...
      console.warn(
        `[duckduckgo_search] Attempt ${attempt}/${retries + 1} failed for "${query}": ${errMsg} (Reason: ${reason})`,
      );
      ...
      throw new Error(
        `Failed to perform search after ${retries + 1} attempts: ${errMsg}`,
      );
    }
  }
}
...
async function _performResearch(params) {
  ...
  if (!query && (!queries || queries.length === 0)) {
    throw new Error(
      "The 'query' or 'queries' parameter is required for perform_research.",
    );
  }
  ...
  if (task_id) {
    try {
      ...
      if (parentTask) {
        console.log(
          `[Research] Linked to parent task: ${parentTask.title} (ID: ${task_id})`,
        );
        ...
        for (const task of subtasks) {
          if (
            task.tags?.includes("stage-1") ||
            task.title?.includes("Stage 1")
          ) {
            stageTasks.stage1 = task.id;
          } else if (
            task.tags?.includes("stage-2") ||
            task.title?.includes("Stage 2")
          ) {
            stageTasks.stage2 = task.id;
          } else if (
            task.tags?.includes("stage-3") ||
            task.title?.includes("Stage 3")
          ) {
      ...
      console.warn(
        `[Research] Failed to get task information: ${error.message}`,
      );
    }
  }
  ...
  function extractKeywordsAndGenerateQueries(
    baseQuery,
    additionalQueries = [],
    maxQueries = 5,
  ) {
    console.log(
      `[Research Stage 1] Generating queries from base: "${baseQuery}"`,
    );
    ...
    if (concepts.length >= 2) {
      for (let i = 0; i < concepts.length - 1; i++) {
        for (let j = i + 1; j < concepts.length; j++) {
          const focusedQuery =
            `${concepts[i]} ${concepts[j]} ${baseQuery.includes("how") || baseQuery.includes("what") ? baseQuery.split(" ").slice(0, 3).join(" ") : ""}`.trim();
    ...
    console.log(
      `[Research Stage 1] Generated ${finalQueries.length} search queries:`,
      finalQueries,
    );
    return finalQueries;
  }
  ...
  function scoreUrlRelevance(url, title, snippet, searchQuery) {
    ...
    if (
      ...
      lowerSnippet.includes(searchQuery)
    ) {
      score += 0.15;
    }
  ...
  async function executeSequentialSearches(searchQueries) {
    console.log(
      `[Research Stage 1] Executing ${searchQueries.length} sequential searches`,
    );
    ...
    for (let i = 0; i < searchQueries.length; i++) {
      ...
      try {
        appendMessage(
          ...
          `🔍 Search ${i + 1}/${searchQueries.length}: "${query}"`,
          "ai",
        );

        const results = await _duckduckgoSearch({
          query,
          parentSignal: researchController.signal,
        });
        ...
        const mappedResults = results.results.map((result) => ({
          ...
          query: query,
          relevanceScore: scoreUrlRelevance(
            result.link,
            result.title,
            result.snippet,
            query,
        ...
        console.error(
          `[Research Stage 1] Search failed for "${query}":`,
          error.message,
        );
        allSearchResults.push([]);
      }
    ...
    console.log(
      `[Research Stage 1] Aggregated ${uniqueResults.length} unique URLs from all searches`,
    );
  ...
  function shouldReadUrl(urlInfo, stage) {
    ...
    console.log(
      `[Research Stage ${stage}] URL: ${urlInfo.url} | Score: ${urlInfo.relevanceScore.toFixed(2)} | Threshold: ${stageThreshold.toFixed(2)} | Read: ${shouldRead}`,
    );
  ...
  async function processUrl(urlInfo, stage) {
    ...
    try {
      appendMessage(
        document.getElementById("chat-messages"),
        `📖 Reading: ${urlInfo.title || urlInfo.url} (Stage ${stage})`,
        "ai",
      );

      const urlContent = await _readUrl({
        url: urlInfo.url,
        parentSignal: researchController.signal,
      });
      ...
      if (!urlContent.content || !urlContent.content.trim()) {
        console.warn(
          `[Research Stage ${stage}] No content found for URL: ${urlInfo.url}`,
        );
        return null;
      }
      ...
      const contentPreview = (contentEntry.content || "")
        .substring(0, 100)
        .replace(/\s+/g, " ")
        .trim();
      console.log(
        `[Research Stage ${stage}] Successfully read content from ${urlInfo.url} | Preview: "${contentPreview}..."`,
      );

      // Phase 1: Add discovered links to the frontier for recursive traversal
      if (
        contentEntry.links.length > 0 &&
        urlInfo.depth < researchState.maxDepth
      ) {
        const newUrlInfos = contentEntry.links.map((link) => {
          ...
          const syntheticQuery = link.text
            ? `${researchState.originalQuery} ${link.text}`
      ...
      console.warn(
        `[Research Stage ${stage}] Failed to read URL ${urlInfo.url}:`,
        error.message,
      );
  ...
  function extractKeywordsFromContent(contentEntry) {
    ...
    const frequentTerms = Object.entries(wordFreq)
      .filter(([word, freq]) => freq >= 5 && !queryTerms.includes(word))
      .sort(([, a], [, b]) => b - a)
      .slice(0, 5)
      .map(([word]) => word);
  ...
  function analyzeContentAndIdentifyGaps() {
    console.log(
      `[Research Stage 2] Analyzing ${researchState.allContent.length} content items from Stage 1`,
    );
    ...
    console.log(
      `[Research Stage 2] Identified ${knowledgeGaps.length} knowledge gaps:`,
  ...
  async function performFocusedReading(gapQueries) {
    console.log(
      `[Research Stage 3] Performing focused reading for ${gapQueries.length} knowledge gaps`,
    );
    ...
    for (const query of gapQueries) {
      try {
        appendMessage(
          document.getElementById("chat-messages"),
          `🔍 Focused search: "${query}" (Stage 3)`,
          "ai",
        );

        const searchResults = await _duckduckgoSearch({
          query,
          parentSignal: researchController.signal,
        });
        ...
        if (!searchResults.results || searchResults.results.length === 0) {
          console.log(
            `[Research Stage 3] No results for gap query: "${query}"`,
          );
          continue;
        }
        ...
        console.error(
          `[Research Stage 3] Search failed for gap query "${query}":`,
          error.message,
        );
        continue;
      }
  ...
  async function executeResearch() {
    try {
      ...
      logger.log(
        `🔬 Stage 1: Performing broad search and exploration (Depth: ${researchState.maxDepth})...`,
      );
      ...
      if (
        researchState.frontier.length === 0 &&
        /^[a-z0-9\-_]{4,24}$/i.test(query)
      ) {
        logger.warn(
          "[Research] No search results found. Applying heuristic platform fallback for potential username.",
        );
        const platformTemplates = [
          {
            url: `https://github.com/${query}`,
            title: `GitHub Profile: ${query}`,
            relevance: 0.95,
          },
          {
            url: `https://www.linkedin.com/in/${query}`,
            title: `LinkedIn Profile: ${query}`,
            relevance: 0.9,
          },
          {
            url: `https://x.com/${query}`,
            title: `X/Twitter Profile: ${query}`,
            relevance: 0.85,
          },
          {
            url: `https://www.instagram.com/${query}`,
            title: `Instagram Profile: ${query}`,
            relevance: 0.8,
          },
          {
            url: `https://dev.to/${query}`,
            title: `Dev.to Profile: ${query}`,
            relevance: 0.88,
          },
        ];
        ...
        logger.log(
          `[Research] Added ${heuristicUrls.length} heuristic platform URLs to the frontier.`,
        );
      }
      ...
      while (
        researchState.frontier.length > 0 &&
        researchState.totalUrlsRead < researchState.maxTotalUrls
      ) {
        if (researchController.signal.aborted) {
          logger.warn("[Research] Deadline exceeded, stopping research.");
          break;
        }
        const processingBatch = researchState.frontier.splice(
          0,
          researchState.concurrentReads,
        );
      ...
      console.log(
        `[Research] Stage 1 (Recursive Traversal) complete. Processed ${researchState.allContent.length} content items.`,
      );
      ...
      if (researchState.taskTools && researchState.stageTasks.stage1) {
        ...
        if (researchState.stageTasks.stage2) {
          await researchState.taskTools.update(
            researchState.stageTasks.stage2,
            {
              status: "in_progress",
              startTime: Date.now(),
            },
      ...
      logger.log(
        `🔬 Stage 2: Analyzing content and identifying knowledge gaps...`,
      );
      ...
      console.log(
        `[Research] Stage 2 complete. Identified ${gapQueries.length} knowledge gaps.`,
      );
      ...
      if (researchState.taskTools && researchState.stageTasks.stage2) {
        ...
        if (researchState.stageTasks.stage3) {
          await researchState.taskTools.update(
            researchState.stageTasks.stage3,
            {
              status: "in_progress",
              startTime: Date.now(),
            },
      ...
      console.log(
        `[Research] Stage 3 complete. Final content count: ${researchState.allContent.length}.`,
      );
      ...
      logger.log(
        `✅ Research completed! Processed ${researchState.allContent.length} sources across 3 stages.`,
      );

      if (researchState.taskTools && researchState.taskId) {
        try {
          const parentTask = researchState.taskTools.getById(
            researchState.taskId,
          );
          if (parentTask) {
            await researchState.taskTools.update(researchState.taskId, {
              ...
              context: {
                ...
                uniqueDomains: new Set(
                  researchState.references.map((url) => {
                    try {
          ...
          console.warn(
            `[Research] Failed to update parent task: ${taskError.message}`,
          );
        }
      }
      ...
      if (error.name === "AbortError") {
        logger.warn(
          `[Research] Aborted due to timeout after ${deadline_ms}ms.`,
        );
        ...
        if (researchState.allContent.length > 0) {
          ...
          try {
            console.warn(
              "[perform_research][result:degraded:timeout:json]",
              JSON.stringify(result, null, 2),
            );
          } catch (_) {
            /* ignore circular refs */
      ...
      if (researchState.taskTools) {
        ...
        try {
          ...
          console.warn(
            `[Research] Failed to update task statuses: ${taskError.message}`,
          );
        }
      }
  ...
  function compileResults({ status = "Success", reason = null } = {}) {
    const successfulContent = researchState.allContent.filter(
      (item) => !item.error,
    );
    ...
    const failureRatio =
      totalAttempts > 0 ? failedUrls.length / totalAttempts : 0;
    // More lenient on small samples, stricter on larger ones.
    const dynamicThreshold = Math.min(
      0.6,
      0.2 + 0.15 * Math.log10(Math.max(1, totalAttempts)),
    );
    const qualityCompromised = failureRatio > dynamicThreshold;

    if (qualityCompromised && status !== "Degraded") {
      throw new Error(
        `Research quality compromised: ${failedUrls.length} out of ${totalAttempts} URL reads failed (ratio ${failureRatio.toFixed(2)} > threshold ${dynamicThreshold.toFixed(2)}).`,
      );
    }
...
- Knowledge gaps identified: ${researchState.knowledgeGaps.length}`;
    ...
    const fullContent = successfulContent
      .map(
        (item) =>
    ...
    return {
      ...
      results: {
        ...
        stats: {
          ...
          degradedReason: qualityCompromised
            ? "High failure rate in URL reads"
            : reason,
        },
        queries: {
          initial: researchState.searchQueries,
          gap: researchState.knowledgeGaps.map(
            (g) => `${researchState.originalQuery} ${g.keyword}`,
          ),
        },
      },
      metadata: {
...
export function registerWebResearchTools() {
  ToolRegistry.register("read_url", {
    ...
    description:
      'Reads and extracts the main content and all links from a given URL. The result will be a JSON object with "content" and "links" properties.',
  ...
  ToolRegistry.register("perform_research", {
    ...
    createsCheckpoint: false,
    description:
      "🔬 ENHANCED: Performs intelligent, recursive web research with AI-driven decision making. Automatically searches, analyzes content relevance, follows promising links, and expands searches based on discovered information. Much more comprehensive than simple search.",
    parameters: {
      query: {
        type: "string",
        required: true,
      ...
      max_results: {
        type: "number",
        description: "Maximum URLs to read per search (1-5, default: 3)",
      },
      depth: {
        type: "number",
        description:
          "Maximum recursion depth for following links (1-3, default: 2)",
      },
      relevance_threshold: {
        type: "number",
        description:
          "Minimum relevance score to read URLs (0.3-1.0, default: 0.7). Lower = more URLs read",
      },
      deadline_ms: {
        type: "number",
        description:
          "Overall timeout in milliseconds for the entire research task (default: 45000)",
      },
    },
  });
}
